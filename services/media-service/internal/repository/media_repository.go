...
func (r *PostgresMediaRepository) CreateMedia(ctx context.Context, media *models.Media) error {
	...
	// Insert media record
	query := `
		INSERT INTO media (id, user_id, filename, content_type, size, bucket, object_name, created_at, updated_at)
		VALUES (:id, :user_id, :filename, :content_type, :size, :bucket, :object_name, :created_at, :updated_at)
	`
...
func (r *PostgresMediaRepository) GetMediaByID(ctx context.Context, id string) (*models.Media, error) {
	...
	query := `
		SELECT id, user_id, filename, content_type, size, bucket, object_name, created_at, updated_at
		FROM media
		WHERE id = $1
	`
