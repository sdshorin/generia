...
func (r *PostgresWorldRepository) Create(ctx context.Context, world *models.World) error {
	query := `
               INSERT INTO worlds (name, description, prompt, creator_id, generation_status, status, image_uuid, icon_uuid, params)
...
func (r *PostgresWorldRepository) GetByID(ctx context.Context, id string) (*models.World, error) {
	query := `
               SELECT id, name, description, prompt, creator_id, generation_status, status, image_uuid, icon_uuid, params, created_at, updated_at
               FROM worlds
               WHERE id = $1
       `
...
func (r *PostgresWorldRepository) GetByUser(ctx context.Context, userID string, limit, offset int, status string) ([]*models.World, int, error) {
	...
	if status != "" && status != "all" {
		query = `
                       SELECT w.id, w.name, w.description, w.prompt, w.creator_id, w.generation_status, w.status, w.image_uuid, w.icon_uuid, w.params, w.created_at, w.updated_at
			FROM worlds w
